/**
 * @file Goldman Sachs Watchlist Application
 * Client-side static feed viewer for api/goldmansachs/** with multi-ETF Watchlist
...
type IndexFund = {
  ...
  cusip?: string | null;
  isin?: string | null;
  frequencyCode?: string | null; // coded cadence label written by scripts/update-data.ts ('04 - Quarterly', …)
...
const INDEX_URL = './api/goldmansachs/index.json';
const THEME_KEY = 'goldmansachs-theme';
const SELECTED_KEY = 'goldmansachs-selected-etfs';
const BLACKLIST_KEY = 'goldmansachs-blacklisted-etfs';
const ACTIVE_FUND_KEY = 'goldmansachs-active-fund';
const FILTERS_KEY = 'goldmansachs-tab-filters';
const LEGACY_FILTERS_KEY = 'goldmansachs-searches'; // pre-rename key, migrated at boot
const SORTS_KEY = 'goldmansachs-tab-sorts';
const SITE_STATE_KEY = 'goldmansachs-site-state';
...
const COLUMN_TOOLTIPS: Record<string, string> = {
  ...
  'Fund Name': 'Fund Name — Official Goldman Sachs Asset Management fund-page name of the exchange-traded fund (ETF).',
  Category: 'Category — the Goldman Sachs Asset Management fund finder groups its ETFs by asset class (Equity, Fixed Income, Commodities); the tab shows that asset class, the fund-finder grouping is kept in meta.json.',
  Name: 'Security Name — Full registered legal name of the company or underlying financial asset.',
  Identifier: 'Identifier — CUSIP / ISIN / FIGI as published in the SEC N-PORT-P filing that supplied the sheet (am.gs.com publishes only top-10 holdings, not a full file). Positions without an exchange ticker (bonds, cash, futures) are identified in the Watchlist by this.',
  ...
  Type: 'Category — the asset class from the Goldman Sachs Asset Management fund finder (see the Category column). Same source as the category tabs.',
  Expense: 'Gross Expense Ratio — Total annual fund operating expenses as a % of assets.',
  'Dividend Yield': 'Dividend Yield — the 12 Month Trailing Distribution Rate published on the Goldman Sachs Asset Management fund page when present; otherwise indicated (latest distribution per share x payments per year / NAV) from the official distribution history.',
  'SEC Yield': 'SEC Yield (30-Day) — The 30-day SEC yield as published on the official Goldman Sachs Asset Management fund page; "—" when that page does not publish one.',
  'YTD Return': 'YTD Return — Official Goldman Sachs NAV total return year-to-date (fund page, month-end series) where published; otherwise market-price return since the start of the year computed from adjusted closes (Yahoo).',
  'TR 1Y': 'TR 1Y (1-Year Total Return) — Official Goldman Sachs NAV total return over the past year (fund page, month-end series) where published, otherwise adjusted market-price return from Yahoo.',
  ...
  'CAGR 3Y': 'CAGR 3Y (3-Year Compound Annual Growth Rate) — Official Goldman Sachs annualized NAV total return over 3 years (fund page) where published, otherwise computed from adjusted closes.',
  'CAGR 5Y': 'CAGR 5Y (5-Year Compound Annual Growth Rate) — Official Goldman Sachs annualized NAV total return over 5 years (fund page) where published, otherwise computed from adjusted closes.',
  'CAGR 10Y': 'CAGR 10Y (10-Year Compound Annual Growth Rate) — Official Goldman Sachs annualized NAV total return over 10 years (fund page) where published, otherwise computed from adjusted closes.',
  YTD: 'YTD total return — official Goldman Sachs NAV return (month-end) where published, otherwise market-price return from adjusted closes.',
  '1Y': '1-year official Goldman Sachs NAV total return where published, otherwise adjusted market-price return, month-end series.',
  '3Y': '3-year average annual return (CAGR), official Goldman Sachs NAV total return where published, otherwise Yahoo adjusted close.',
  '5Y': '5-year average annual return (CAGR), official Goldman Sachs NAV total return where published, otherwise Yahoo adjusted close.',
  '10Y': '10-year average annual return (CAGR), official Goldman Sachs NAV total return where published, otherwise Yahoo adjusted close.',
  'SI Ann.': 'Since-inception annualized return, official Goldman Sachs NAV total return where published, otherwise Yahoo adjusted close.',
  ...
  Frequency: 'Frequency — sortable payment cadence coded by scripts/update-data.ts (frequencyCode in index.json) from the official Goldman Sachs distribution history, Yahoo dividends as fallback: 01 - Monthly, 04 - Quarterly, 06 - Semi-annually, 12 - Annually; 00 denotes unavailable/unknown and 99 denotes irregular.',
  'Ex-Date': 'Ex-dividend date of the latest distribution.',
  Dividend: 'Latest dividend per share.',
  Coupon: 'Bond annual coupon rate (%).',
  Maturity: 'Bond maturity date.',
  'Market Value': 'Position market value in USD — as reported by the SEC N-PORT-P filing that supplied the sheet (the fund page publishes only the top-10 weights, not full position values).',
...
function exportFileName(scope: string, extension: string): string {
  ...
  return `goldmansachs-${scope.toLowerCase().replace(/\s+/g, '-')}-${stamp}.${extension}`;
}
...
// =========================================================================
// 4. Static API loading & paginated sheets (api/goldmansachs/**, iShares-style)
// =========================================================================
...
function normalizeFundRow(fund: IndexFund): FundRow {
  ...
  const row: any = {
    ...
    secYield: metrics.secYield ?? null, // official Goldman Sachs fund pages publish the 30-day SEC yield for most funds.
...
async function loadCatalog(): Promise<void> {
  setStatus('Loading Goldman Sachs ETF data from api/goldmansachs/index.json…', 'info');
...
async function loadFundMeta(ticker: string): Promise<any> {
  ...
  const request = (async () => {
    try {
      const meta = await fetchJson(`./api/goldmansachs/funds/${encodeURIComponent(ticker)}/meta.json`);
...
async function fetchPage(ticker: string, pagePath: string): Promise<{ headers: string[]; rows: string[][] }> {
  ...
  const page = await fetchJson(`./api/goldmansachs/funds/${encodeURIComponent(ticker)}/${path}`);
  ...
  if (!state.selected.size) {
    ...
    el.tableBody.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400 dark:text-slate-500">${
      loading
        ...
        : 'Holdings data is not available yet. Run bun ./scripts/update-data.ts to refresh the Goldman Sachs feed.'
...
function renderOverviewTable(fund: FundRow): void {
  ...
  const overview: Array<{ section: string; metric: string; value: unknown }> = [
    ...
    { section: 'Distributions', metric: 'SEC Yield (30-day)', value: meta && meta.yields ? (meta.yields.secYieldText || '—') : (fund.secYield === null || fund.secYield === undefined ? 'not published on the official Goldman Sachs Asset Management fund page for this fund' : `${fund.secYield.toFixed(2)}%`) },
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} overview · ${rows.length} metrics. Returns are the official Goldman Sachs NAV total returns where published (Yahoo adjusted closes otherwise).`);
}
...
function renderDistributionsTable(fund: FundRow): void {
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} distributions · official Goldman Sachs distribution history (ex-date, distribution per share; Yahoo dividends when the fund-page table is unavailable); frequency is inferred from the cadence.`);
}
...
function renderSubtitle(text?: string): void {
  ...
  const countsText = state.counts
    ...
    : '';
  const base = text ? String(text) : 'Search Goldman Sachs ETFs, select ETFs via the “Use” checkbox, then use the Watchlist tab.';
  ...
  el.subtitle.innerHTML = `
    ...
    <span class="block sm:inline">·${generated ? ` updated ${escapeHtml(generated)}` : ''}${countsText ? ` · ${escapeHtml(countsText)}.` : '.'} Data: <a href="./api/goldmansachs/index.json" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">api/goldmansachs/index.json</a> generated from <a href="https://am.gs.com/en-us/individual/funds?locale=en-us&audience=individual&sf=funds&filters=funds%7CETF&limit=100" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">am.gs.com fund pages</a> + SEC N-PORT-P + Yahoo Finance</span>
  `;
...
function init(): void {
  ...
  void loadCatalog().catch(error => {
    ...
    setStatusRow(`Unable to load api/goldmansachs/index.json: ${message}. Run bun ./scripts/update-data.ts and serve the folder (for example bunx serve . -p 1234).`, 'error');
  });
}
